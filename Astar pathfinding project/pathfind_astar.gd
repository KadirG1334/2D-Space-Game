...
export(Vector2) var map_size = Vector2(35, 12)
...
# define the obstacles one by one as array to Tilemap 
...
# Once you added all points to the AStar node, we've got to connect them
# The points don't have to be on a grid: we can use this class
# to create walkable graphs.
...
#Recalculating to path for every state changes
func _recalculate_path():
	clear_previous_path_drawing()
	var start_point_index = calculate_point_index(path_start_position)
	var end_point_index = calculate_point_index(path_end_position)
	# This method gives us an array of points. Need the start and end
	# points' indices as input
	_point_path = astar_node.get_point_path(start_point_index, end_point_index)
	# Redraw the lines and circles from the start to the end point
	update()#updating on the 2D
